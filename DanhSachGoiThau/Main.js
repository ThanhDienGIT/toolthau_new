...
const formatgetToDay =  () => {
  const ngay =  today.getDate() > 9 ? today.getDate() : '0' + String(today.getDate())
  const thang =   (today.getMonth() + 1) > 9 ?(today.getMonth() + 1) : '0' + String((today.getMonth() + 1))
...
// Hàm lấy dữ liệu tu ngay den ngay
const FunctionMainDrawInsert = async (driver,j) => {
  console.time('get')
  try{
    ...
    for (let el of dulieucao) {
      ...
      const divLinhVuc = await el.findElement(By.className('col-md-4 content__body__left__item__infor__contract__other'))      
      ...
      //     await bot.telegram.sendMessage(-449294338, strMessage ,{
      //       parse_mode: "Markdown",
      //       reply_markup: {
...
const sendKeyToGetFuntionFollowWant = async (keyVaklue) => {
...
const GetListPackageEveryDay = async () => {
  console.log("Start");
  ...
  try {
    ...
    await driver.get("https://muasamcong.mpi.gov.vn/web/guest/contractor-selection?render=index");
    ...
    await bot.telegram.sendMessage(
      6073926430,
      `Đã lấy xong ngày ${formatgetToDay()} + Tổng gói thầu mới là : ${j}`
    );
  } catch (err) {
    console.log(err);
